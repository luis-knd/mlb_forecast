...
class SchedulerUseCases:
    ...
    async def generate_upcoming_predictions(self) -> Dict[str, Any]:
        ...
        try:
            # Get upcoming games for the next 3 days - use list_upcoming_games instead
            upcoming_games = await self.game_repository.list_upcoming_games(days_ahead=3)

            predictions_generated = 0

            for game in upcoming_games:
                try:
                    # Validate game.id is not None before proceeding
                    if game.id is None:
                        continue

                    # Check if prediction already exists - use list_by_game instead
                    existing_predictions = await self.prediction_repository.list_by_game(game.id)
                    ...
                    if not existing_predictions:
                        ...
                        if home_team_stats and away_team_stats:
                            ...
                            prediction = await self.ml_model.predict_game_outcome(
                                home_team_stats, away_team_stats, game.game_date
                            )

                            # Set game ID
                            prediction.game_id = game.id

                            # Save prediction using save method
                            await self.prediction_repository.save(prediction)

                            # Create dict manually since to_dict might not exist
                            prediction_dict = {
                            ...
                            await self.cache.set(
                                f"mlb:prediction:{game.id}:outcome",
                                prediction_dict,
                                ttl=1800,  # 30 minutes
                            )
                    ...
                    logger.warning(f"Error generating prediction for game {game.id}: {e}")
                    continue
    ...
    async def _ingest_games_for_date(self, date_obj: date) -> List[Game]:
        ...
        """
        # Get games from API
        games_data = await self.mlb_api.get_games_by_date(date_obj)
        ...
        for game_data in games_data:
            ...
            # Create Game entity - remove invalid arguments and ensure proper types
            game = Game.create(
            ...
            # Save to repository - use save instead of create_or_update
            await self.game_repository.save(game)
            games.append(game)

            # Cache game data - use ttl instead of expire
            game_dict = {
                "id": game.id,
            ...
            await self.cache.set(f"game:{game.mlb_game_id}", game_dict, ttl=3600)
    ...
    def _create_game_features(
        ...
        # Convert TeamStats entities to dictionaries manually
        home_stats_dict = {
            "games_played": _get_n(home_hitting, "games_played"),
            "wins": _get_n(home_pitching, "wins"),
            "losses": _get_n(home_pitching, "losses"),
            "runs_scored": _get_n(home_hitting, "runs_scored"),
            "runs_allowed": _get_n(home_pitching, "runs_allowed"),
            "batting_average": _get_f(home_hitting, "batting_average"),
            "on_base_percentage": _get_f(home_hitting, "on_base_percentage"),
            "slugging_percentage": _get_f(home_hitting, "slugging_percentage"),
            "earned_run_average": _get_f(home_pitching, "earned_run_average"),
            "win_percentage": (_get_n(home_pitching, "wins") / max(_get_n(home_hitting, "games_played"), 1)),
            "ops": _get_f(home_hitting, "ops"),
            "run_differential": _get_n(home_hitting, "runs_scored") - _get_n(home_pitching, "runs_allowed"),
        }

        away_stats_dict = {
            "games_played": _get_n(away_hitting, "games_played"),
            "wins": _get_n(away_pitching, "wins"),
            "losses": _get_n(away_pitching, "losses"),
            "runs_scored": _get_n(away_hitting, "runs_scored"),
            "runs_allowed": _get_n(away_pitching, "runs_allowed"),
            "batting_average": _get_f(away_hitting, "batting_average"),
            "on_base_percentage": _get_f(away_hitting, "on_base_percentage"),
            "slugging_percentage": _get_f(away_hitting, "slugging_percentage"),
            "earned_run_average": _get_f(away_pitching, "earned_run_average"),
            "win_percentage": (_get_n(away_pitching, "wins") / max(_get_n(away_hitting, "games_played"), 1)),
            "ops": _get_f(away_hitting, "ops"),
            "run_differential": _get_n(away_hitting, "runs_scored") - _get_n(away_pitching, "runs_allowed"),
        }

        # Create basic features
        features = {
            # Home team features
            "home_games_played": home_stats_dict.get("games_played", 0),
            "home_wins": home_stats_dict.get("wins", 0),
            "home_losses": home_stats_dict.get("losses", 0),
            "home_runs_scored": home_stats_dict.get("runs_scored", 0),
            "home_runs_allowed": home_stats_dict.get("runs_allowed", 0),
            "home_batting_average": home_stats_dict.get("batting_average", 0.0),
            "home_on_base_percentage": home_stats_dict.get("on_base_percentage", 0.0),
            "home_slugging_percentage": home_stats_dict.get("slugging_percentage", 0.0),
            "home_earned_run_average": home_stats_dict.get("earned_run_average", 0.0),
            "home_win_percentage": home_stats_dict.get("win_percentage", 0.0),
            "home_ops": home_stats_dict.get("ops", 0.0),
            "home_run_differential": home_stats_dict.get("run_differential", 0),
            # Away team features
            "away_games_played": away_stats_dict.get("games_played", 0),
            "away_wins": away_stats_dict.get("wins", 0),
            "away_losses": away_stats_dict.get("losses", 0),
            "away_runs_scored": away_stats_dict.get("runs_scored", 0),
            "away_runs_allowed": away_stats_dict.get("runs_allowed", 0),
            "away_batting_average": away_stats_dict.get("batting_average", 0.0),
            "away_on_base_percentage": away_stats_dict.get("on_base_percentage", 0.0),
            "away_slugging_percentage": away_stats_dict.get("slugging_percentage", 0.0),
            "away_earned_run_average": away_stats_dict.get("earned_run_average", 0.0),
            "away_win_percentage": away_stats_dict.get("win_percentage", 0.0),
            "away_ops": away_stats_dict.get("ops", 0.0),
            "away_run_differential": away_stats_dict.get("run_differential", 0),
            # Matchup features
            "win_pct_diff": home_stats_dict.get("win_percentage", 0.0) - away_stats_dict.get("win_percentage", 0.0),
            "runs_diff_advantage": home_stats_dict.get("run_differential", 0)
            - away_stats_dict.get("run_differential", 0),
            "ops_diff": home_stats_dict.get("ops", 0.0) - away_stats_dict.get("ops", 0.0),
            "era_diff": away_stats_dict.get("earned_run_average", 0.0) - home_stats_dict.get("earned_run_average", 0.0),
